...
const initHiScore = () => {
	let hiScore: number;
	if (typeof localStorage !== "undefined" && localStorage.getItem("hiScore2")) {
		hiScore = parseInt(localStorage.getItem("hiScore2") as string);
	} else {
		hiScore = 0;
	}
	return hiScore;
...
export default function PlayingPage() {
	...
	let newHiragana = structuredClone(jpchars.hiragana);
	let getChar = () => {
		let rnd = randomNumber(0, Object.keys(newHiragana).length);
		let char = Object.entries(newHiragana).at(rnd);
		if (!char) {
			getChar();
			return;
		}
	...
	let Check = () => {
		...
		if (okays.includes(input)) {
			...
			if (hiScore() < score()) {
				setHiScore(score());
				localStorage.setItem("hiScore2", score().toString());
			}
			setTimeout(() => {
				SetPlayState();
				setTimeout(() => {
					...
					<Switch>
						...
						<Match when={state() == 3}>
							...
							<p>
								...
								Come back later to start again (over 20 minutes)
							</p>
							<div class=" w-screen pt-12 relative">
